...
public abstract class ServiceImpl<E extends BaseIdEntity<E>, V extends BaseIdVo<E, V>> extends BasisServiceImpl<E, V, String>
     ...
     * @param entity
     */
    private void refreshOrderNum(E entity) throws ApplicationException {

        @SuppressWarnings("rawtypes")
        BaseTreeEntity tree = (BaseTreeEntity) entity;
        Integer orderNum = tree.getOrderNum();
        if (null == orderNum) {
            String hql = "select max(t.orderNum) from " + entity.getClass().getSimpleName() + " t";
            Long rs = getDao().findUnipueResultByHQL(hql, null, Long.class);
            if (null == rs) {
