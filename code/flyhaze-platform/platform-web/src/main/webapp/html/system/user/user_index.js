...
$(document).ready(function() {
    ...
    var doRefresh = function() {
        ...
        userGrid.datagrid("reload", queryParams);
    };

    var doQuery = function() {
        queryParams["keyword"] = userSearchbox.searchbox("getValue");
        queryParams["_"] = new Date().getTime();
        userGrid.datagrid("reload", queryParams);
    };
    ...
    var doSubmit = function() {
        engine.confirm("警告", "是否保存用户数据？", function(r) {
            if (r) {
    ...
    var showDetailDialog = function() {
        var url = "./html/system/user/user_detail.html";
        detailDialog = engine.showDialog({
            ...
            onLoad: function() {
                userForm = $("#user_dialog").find("form");
            }
        });
    ...
    var doDel = function(id) {
        engine.confirm("警告", "是否删除用户数据？", function(r) {
            if (r) {
                var ids = [];
                if (id) {
                    ids[0] = id;
                    ...
                    if (rows && length > 0) {
                        ...
                        engine.messager("警告", "必须至少选择一个要删除的用户！");
                    };
                };
    ...
    // 查询条件菜单文本输入框
    var userSearchbox = $("#userSearchbox");
